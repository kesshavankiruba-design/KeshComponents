...
// Gemini-backed PCB generation and placement.
...
function renderGeneratedBoard(plan){
  ...
  $('.board-status').innerHTML=`<span><i class="pulse"></i> Gemini plan · ${components.length} parts</span><span>${areaW} × ${areaH} mm</span>`;
  updateConnections();
}
...
$('#generateBtn').onclick=async()=>{
  ...
  try{
    label.textContent='Sending requirements to Gemini…';bar.style.width='20%';pct.textContent='20%';
    const data=await apiPost('/api/generate',{prompt});
    label.textContent='Reading Gemini component plan…';bar.style.width='55%';pct.textContent='55%';
    ...
    $('#optBadge').textContent='Gemini';
    $('#optCopy').textContent=data.plan.optimizationSummary||'Gemini produced a compact conceptual placement.';
    select(data.plan.components[0]?.id);
    bar.style.width='100%';pct.textContent='100%';text.textContent='PCB Generated';
    toast(`${data.plan.components.length} components planned by Gemini`);
  }catch(error){
    console.error(error);
    $('#checkStatus').textContent='Gemini unavailable';
    toast(error.message||'Gemini request failed');
  }finally{
...
async function applyPlacementCommand(){
  ...
  reply.textContent='Gemini is interpreting the placement request…';
  try{
    ...
    if(r.action==='swap'){
      ...
      if(!a||!b){reply.textContent='Gemini returned components that are not on the current board.';return}
      ...
      if(!el){reply.textContent='Gemini returned a component that is not on the current board.';return}
    ...
    console.error(error);reply.textContent=error.message||'Gemini placement request failed.';
  }
}
