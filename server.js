...
function describeOutputShape(schema) {
  if (!schema || typeof schema !== 'object') return 'a JSON value';
  if (schema.type === 'array') return `an array of ${describeOutputShape(schema.items)}`;
  if (schema.type !== 'object') return `a ${schema.type || 'JSON'} value`;
  const fields = Object.entries(schema.properties || {}).map(([name, child]) => `${name}: ${describeOutputShape(child)}`);
  return `an object with ${fields.join('; ')}`;
}

async function structuredGenerate(contents, schema) {
  if (!process.env.OPENROUTER_API_KEY) {
    throw new Error('OPENROUTER_API_KEY is not configured. Add it to .env or your environment.');
  }

  console.log(`OpenRouter request using model: ${AI_MODEL}`);
  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: AI_MODEL,
        // openrouter/free selects a free model that supports JSON mode when
        // this is requested.
        response_format: { type: 'json_object' },
        messages: [{
          role: 'user',
          content: `${contents}\n\nIMPORTANT: Return only the requested JSON data—never return a JSON Schema. Do not include keys named "properties", "required", or "type". Do not use Markdown, code fences, headings, or explanatory text. The result must be ${describeOutputShape(schema)}.`
        }]
      })
    });
    payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      throw new Error(`OpenRouter request failed (status ${response.status}): ${JSON.stringify(payload)}`);
    }
  ...
  try {
    ...
    throw new Error(`OpenRouter returned invalid JSON: ${output.slice(0, 500)}`);
  }
}
...
async function handleApi(req, res) {
  try {
    ...
    if (req.url === '/api/generate') {
      ...
      return sendJson(res, 200, { plan, model: AI_MODEL });
    }
    if (req.url === '/api/placement') {
      ...
      return sendJson(res, 200, { result, model: AI_MODEL });
    }
    ...
    const status = /OPENROUTER_API_KEY/.test(message) ? 500 : 502;
    return sendJson(res, status, { error: message, model: AI_MODEL });
  }
}
...
server.listen(PORT, () => {
  ...
  console.log(process.env.OPENROUTER_API_KEY ? 'OpenRouter API key detected.' : 'OpenRouter API key not detected yet. Add OPENROUTER_API_KEY to .env.');
});
